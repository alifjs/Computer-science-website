...
function play_snake_game(ctx,canvas){
    let snake = []
    ...
    function grow_snake() {
        ...
        } else if (snake[snake.length - 1].dx < 0) {
            //tail moving left
            snake.push(new Box_constructor(snake[snake.length - 1].y))
            snake[snake.length - 1].x = snake[snake.length - 2].x + 20
            snake[snake.length - 1].dx = -20
            snake[snake.length - 1].dy = 0
        } else if (snake[snake.length - 1].dy < 0) {
            //tail moving up
    ...
    function collision_check() {
        ...
        if (
            snake[0].x < 0 ||
            snake[0].x + 20 > canvas.width ||
            snake[0].y < 0 ||
            snake[0].y + 20 > canvas.height
        ) {
    ...
    let game_over=false
    function move() {
        if (counter % 10 == 0 && counter != 0 && game_over==false) {
            ...
            (snake[0].x+40>=canvas.width || snake[0].x-20<=0||
            snake[0].y-20<=0||snake[0].y+40>=canvas.height)){
                let directions=['ArrowUp','ArrowDown','ArrowRight','ArrowLeft']
                let random_index=Math.floor(Math.random() * 4)
                direction_change(directions[random_index])
            }
        }
        ctx.font = "25px Comic Sans MS";
        ctx.fillStyle='white'
    ...
    function direction_change(direction) {
        if (direction == "ArrowRight" && snake[0].dx == 0 && snake[0].dy != 0) {
            //snake head is not moving to the right/left and isn't static
            right_coordinates.push([snake[0].x, snake[0].y])
        } else if (direction == "ArrowLeft" && snake[0].dx == 0 && snake[0].dy != 0) {
            //snake head is not moving to the right/left and isn't static
            left_coordinates.push([snake[0].x, snake[0].y])
        } else if (direction == "ArrowUp" && snake[0].dy == 0) {
            //snake head isn't moving up/down or it is static
        ...
        } else if (direction == "ArrowDown" && snake[0].dy == 0 && snake[0].dx != 0) {
            //snake head isn't moving up/down and isn't static
    ...
    function change_dx_and_dy() {
        for (let i = 0; i < snake.length; i++) {
            for (let w in right_coordinates) {
                if (
                    snake[i].x == right_coordinates[w][0] &&
                    snake[i].y == right_coordinates[w][1]
                ) {
                    snake[i].dy = 0
                    snake[i].dx = 20
                }
            ...
            for (let w in left_coordinates) {
                if (
                    snake[i].x == left_coordinates[w][0] &&
                    snake[i].y == left_coordinates[w][1]
                ) {
                    snake[i].dy = 0
                    snake[i].dx = -20
                }
            ...
            for (let w in up_coordinates) {
                if (
                    snake[i].x == up_coordinates[w][0] &&
                    snake[i].y == up_coordinates[w][1]
                ) {
                    snake[i].dy = -20
                    snake[i].dx = 0
                }
            ...
            for (let w in down_coordinates) {
                if (
                    snake[i].x == down_coordinates[w][0] &&
                    snake[i].y == down_coordinates[w][1]
                ) {
                    snake[i].dy = 20
                    snake[i].dx = 0
                }
...
export default play_snake_game